...
public final class AdminGui implements GuiListener.ChamoGui {
    ...
    private void handleAction(String action) {
        switch (action) {
            ...
            case "EDITOR" -> {
                new SkinEditorGui(plugin, player, Bukkit.getServicesManager().load(net.chamosmp.chamoitemskins.api.service.SkinService.class)).open();
            }
            case "GRANT", "REVOKE", "GIVE" -> {
                player.sendMessage(MessageUtil.parse("<yellow>Feature " + action + " requires player/skin input. Use commands for now."));
            }
